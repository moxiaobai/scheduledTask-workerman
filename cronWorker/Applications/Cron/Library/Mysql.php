...
class Mysql {
	...
	public static function instance($database) {
		$config = Config::get('application', "mysql.{$database}");
		if(!isset($config)) {
			throw new Exception('Missing Database Config');
		}

		if ( ! isset(self::$_instances[$database]) ) {
			$instance = new Mysql( $config['host'], $config['user'], $config['password'], $config['database'] );
	...
	public function __construct($hostname, $username, $password, $database) {
		if ( ! $this->_connection = mysqli_connect($hostname, $username, $password, $database)) {
			...
			throw new Exception('Error: Could not make a database Connection');
		}
